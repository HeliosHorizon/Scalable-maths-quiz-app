...
function generateQuestion(usedQuestions) {
  ...
  do {
    num1 = Math.floor(Math.random() * 20) + 2; // Start from 2 to ensure num1 >= num2
  } while (usedQuestions.has(`${num1}-2`)); // Avoid repeating questions
...
function MathSubtract2() {
  ...
  useEffect(() => {
    ...
    const generatedQuestions = Array.from({ length: 10 }, () => {
      ...
      usedQuestions.add(`${question.num1}-2`);
      return question;
    });
  ...
  const handleSubmit = () => {
    ...
    setAnswerSummary((prevSummary) => [
      ...prevSummary,
      {
        question: `${questions[currentQuestionIndex].num1} - 2 = `,
